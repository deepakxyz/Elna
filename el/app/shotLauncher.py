...
class ShotLauncher(QtWidgets.QWidget):


	def __init__(self):
		super(ShotLauncher, self).__init__(parent=None)

		self.setWindowTitle('El Shot Launcher')
		self.setMinimumSize(1000,300)
		self.setIcon()

		self.BASE_PATH = os.path.join(os.getcwd())
		# self.BASE_PATH = 'Y:/pipeline/shows/Workbench'
	...
	def setIcon(self):
		appIcon = QtGui.QIcon('Z:/Elan/el/app/icons/head.png')
	...
	def create_widget(self):
		...
		path = os.path.join(self.BASE_PATH,'shots')
		sequences_list = [ name for name in os.listdir(path) if os.path.isdir(os.path.join(path, name)) ]

		self.sequences = QtWidgets.QComboBox()
		self.sequences.addItems(sequences_list)

		self.shots = QtWidgets.QComboBox()

		# read the char asset at the first
		path = os.path.join(self.BASE_PATH,'shots', sequences_list[0])
		shots = [ name for name in os.listdir(path) if os.path.isdir(os.path.join(path, name)) ]
		self.shots.addItems(shots)


		self.build_type = QtWidgets.QComboBox()
		self.build_type.addItems(['animation', 'fx', 'cfx', 'lighting', 'comp', 'render'])
	...
	def create_layout(self):
		...
		form_layout.addRow('Sequences', self.sequences)
		form_layout.addRow('Shots', self.shots)
	...
	def create_connection(self):
		# get asset details
		self.sequences.currentTextChanged.connect(self.sequences_changed)
	...
	def launch_selected_file(self):
		seq = self.sequences.currentText()
		shot = self.shots.currentText()
		self.build_type_data = self.build_type.currentText()

		proj = os.path.join(self.BASE_PATH, seq, shot)

		if self.item_data.endswith('.mb') or shot.endswith('.ma'):
	...
	def launch_blank_command(self):
		sequence = self.sequences.currentText()
		shot = self.shots.currentText()
		self.build_type_data = self.build_type.currentText()
		...
		if self.maya_blk_file.isChecked():
			...
			self.path = os.path.join(self.BASE_PATH, 'shots', sequence, shot)
			...
			path = os.path.join(self.BASE_PATH, 'shots', sequence, shot, self.build_type_data)
			os.chdir(path) 
	...
	def refresh_tree_widget(self):
		...
		# asset type
		sequence_data = self.sequences.currentText()
		shot = self.shots.currentText()
		self.build_type_data = self.build_type.currentText()

		self.path = os.path.join(self.BASE_PATH, 'shots', sequence_data, shot)
	...
	def sequences_changed(self):
		seq = self.sequences.currentText()
		self.shots.clear()
		path = os.path.join(self.BASE_PATH,'shots',  seq)
		shots = [ name for name in os.listdir(path) if os.path.isdir(os.path.join(path, name)) ]
